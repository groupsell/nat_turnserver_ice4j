 ...
 * address it got back or <tt>null</tt> if there was no response.
 *
 * @author Emil Ivov
 */
public class SimpleAddressDetector {
	 ...
	 * 
	 * @param socket
	 *            the socket whose address needs to be resolved.
	 ...
	 *             if we cannot bind the socket.
	 */
	public TransportAddress getMappingFor(IceSocketWrapper socket) throws IOException, BindException {
		TransportAddress localAddress = new TransportAddress((InetSocketAddress) socket.getLocalSocketAddress(),
				Transport.UDP);
		...
		if (evt != null) {
			...
			MappedAddressAttribute maAtt = (MappedAddressAttribute) res.getAttribute(Attribute.MAPPED_ADDRESS);
			if (maAtt != null) {
