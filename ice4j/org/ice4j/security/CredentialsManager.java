 ...
 * {@link CredentialsAuthority} implementation. The point of this mechanism is
 * to allow use in both applications that would handle large numbers of possible
 * users (such as STUN/TURN servers) or others that would only work with a few,
 * like for example an ICE implementation.
 *
 * TODO: just throwing a user name at the manager and expecting it to find an
 * authority that knows about it may lead to ambiguities so we may need to add
 * other parameters in here that would allow us to better select an authority.
 *
 * @author Emil Ivov
...
public class CredentialsManager {
	...
	/**
	 * The list of <tt>CredentialsAuthority</tt>s registered with this manager
	 * as being able to provide credentials. If non-<tt>null</tt>, represents an
	 * unmodifiable view of {@link #authorities}. The field was introduced in
	 ...
	 * {@link CredentialsAuthority}s registered with this manager and and
	...
	private CredentialsAuthority[] getAuthorities() {
		synchronized (authorities) {
			if (unmodifiableAuthorities == null) {
				unmodifiableAuthorities = authorities.toArray(new CredentialsAuthority[authorities.size()]);
			}
	 ...
	 *
	 * @param username
	 *            a local user name or user frag whose credentials we'd like to
	 *            obtain.
	 *
	 * @return <tt>null</tt> if username was not a recognized local user name
	 *         for none of the currently registered
	 *         <tt>CredentialsAuthority</tt>s or a <tt>byte</tt> array
	 *         containing the first non-<tt>null</tt> password that one of them
	 *         returned.
	 */
	 ...
	 *
	 * @param username
	 *            a remote user name or user frag whose credentials we'd like to
	 *            obtain.
	 * @param media
	 ...
	 *
	 * @param authority
	 *            the {@link CredentialsAuthority} to add to this manager.
	 */
	 ...
	 *
	 * @param authority
	 *            the {@link CredentialsAuthority} to remove from this manager.
	 */
