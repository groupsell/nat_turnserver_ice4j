...
class PseudoTcpSocketImpl extends SocketImpl implements PseudoTcpNotify {
	...
	private static final java.util.logging.Logger logger = java.util.logging.Logger
			.getLogger(PseudoTcpSocketImpl.class.getName());
	/**
	 ...
	 *
	 * @param conv_id
	 *            conversation id, must be the same on both sides
	 * @param sock
	 *            datagram socket used for network operations
	 */
	public PseudoTcpSocketImpl(long conv_id, DatagramSocket sock) {
		...
		/*
		 * try { setOption(SO_TIMEOUT, 0); } catch (SocketException e) { throw
	 ...
	 *            conversation id, must be the same on both sides
	 * @param local_port
	 *            the local port that will be used for this socket
	 * @throws SocketException
	 */
	 ...
	 *
	 * @param remoteAddress
	 *            to which this socket connects to
	 * @param timeout
	 *            for this operation in ms
	 * @throws IOException
	 */
	void doConnect(InetSocketAddress remoteAddress, long timeout) throws IOException {
		...
		try {
			...
			synchronized (state_notify) {
				while (pseudoTcp.getState() != PseudoTcpState.TCP_ESTABLISHED && (noTimeout || (elapsed < timeout))) {
	 ...
	 *
	 * @param remoteAddress
	 *            the one and only address that will be accepted as the source
	 *            for remote packets
	 * @param timeout
	 *            for this operation in ms
	 * @throws IOException
	 *             If socket gets closed or timeout expires
	 */
	...
	/**
	 * Implements <tt>PseudoTcpNotify</tt> Called when TCP enters connected
	 * state.
	 *
	 * @param tcp
	 *            the {@link PseudoTCPBase} that caused an event
	 ...
	 *            the {@link PseudoTCPBase} that caused an event
	 * @param e
	 *            the <tt>Exception</tt> which is the reason for closing socket,
	...
	public WriteResult tcpWritePacket(PseudoTCPBase tcp, byte[] buffer, int len) {
		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST, "write packet to network length " + len + " address " + remoteAddr);
		}
		try {
			// TODO: in case the packet is too long it should return
			// WR_TOO_LARGE
		...
		while (runReceive) {
			try {
				...
				if (remoteAddr == null) {
					...
					logger.log(Level.WARNING, "Remote addr not set previously, setting to " + remoteAddr);
				} else {
					if (!packet.getSocketAddress().equals(remoteAddr)) {
						logger.log(Level.WARNING, "Ignoring packet from " + packet.getAddress() + ":" + packet.getPort()
	...
	private final static ScheduledThreadPoolExecutor clockExecutor = new ScheduledThreadPoolExecutor(1);
	...
	private void runClock() {
		...
		if (sleep == -1) {
			releaseAllLocks();
			if (exception != null) {
				logger.log(Level.SEVERE, "STATE: " + pseudoTcp.getState() + " ERROR: " + exception.getMessage());
			}
		} else {
	...
	private void scheduleClockTask(long sleep) {
		synchronized (clockTaskRunner) {
			...
			if (runClock) {
				currentlyScheduledClockTask = clockExecutor.schedule(clockTaskRunner, sleep, TimeUnit.MILLISECONDS);
			}
		}
	}
	 ...
	 *         blocking.
	 * @throws IOException
	 *             if an I/O error occurs when determining the number of bytes
	 *             available.
	 */
	...
	public void close() throws IOException {
		try {
			...
			// System.out.println("ON CLOSE: in flight
			// "+pseudoTcp.GetBytesInFlight());
			// System.out.println("ON CLOSE: buff not sent
			// "+pseudoTcp.GetBytesBufferedNotSent());
	...
	class PseudoTcpInputStream extends InputStream {
		 ...
		 * @param length
		 *            maximum count of bytes that can be read
		 * @return byte count actually read
		 * @throws IOException
		 *             in case of error or if timeout occurs
		 */
		@Override
			...
			while (true) {
				...
				try {
					...
					if (readTimeout > 0) {
						...
						if (left <= 0) {
							IOException exc = new IOException("Read operation timeout");
					...
					if (logger.isLoggable(Level.FINER)) {
						logger.log(Level.FINER, "Read notified: " + pseudoTcp.getAvailable());
					}
					if (exception != null) {
						throw exception;
					}
			...
			while (toSend > 0) {
				...
				if (sent > 0) {
					...
					try {
						...
						logger.log(Level.FINER, "Write notified, available: " + pseudoTcp.getAvailableSendBuffer());
						if (exception != null) {
							throw exception;
						}
					} catch (InterruptedException ex) {
