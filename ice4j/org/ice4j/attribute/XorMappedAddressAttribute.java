...
/**
 * The XOR-MAPPED-ADDRESS attribute is only present in Binding Responses. It
 * provides the same information that is present in the MAPPED-ADDRESS
 * attribute. However, the information is encoded by performing an exclusive or
 * (XOR) operation between the mapped address and the transaction ID.
 * Unfortunately, some NAT devices have been found to rewrite binary encoded IP
 * addresses and ports that are present in protocol payloads. This behavior
 * interferes with the operation of STUN. By providing the mapped address in an
 * obfuscated form, STUN can continue to operate through these devices.
 *
 ...
 * (Variable) +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * The Family represents the IP address family, and is encoded identically to
 * the Family in MAPPED-ADDRESS.
 *
 * X-Port is equal to the port in MAPPED-ADDRESS, exclusive or'ed with most
 * significant 16 bits of the transaction ID. If the IP address family is IPv4,
 * X-Address is equal to the IP address in MAPPED- ADDRESS, exclusive or'ed with
 * the most significant 32 bits of the transaction ID. If the IP address family
 * is IPv6, the X-Address is equal to the IP address in MAPPED-ADDRESS,
 * exclusive or'ed with the entire 128 bit transaction ID.
 *
 * @author Emil Ivov
 */
