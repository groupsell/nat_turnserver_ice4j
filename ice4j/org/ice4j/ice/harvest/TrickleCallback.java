...
public interface TrickleCallback {
	 ...
	 *
	 * @param iceCandidates
	 *            the newly discovered list of candidates or, similarly to
	 *            WebRTC, <tt>null</tt> in case all candidate harvesting is now
	 *            completed.
	 */
	 ...
	 * stream. This may allow for optimizations in cases where all checks for a
	 * specific stream have failed and no candidate has been found, so a failure
	 * is due and we wouldn't won't to waist any more time.
	 *
	 * @param stream
	 *            the {@link IceMediaStream}, for which we have just completed
	 *            harvesting.
	 */
