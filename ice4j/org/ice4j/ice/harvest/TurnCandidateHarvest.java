...
public class TurnCandidateHarvest extends StunCandidateHarvest {
	 ...
	 *
	 * @param harvester
	 *            the <tt>TurnCandidateHarvester</tt> which is performing the
	 *            TURN harvesting
	 * @param hostCandidate
	 *            the <tt>HostCandidate</tt> for which TURN <tt>Candidate</tt>s
	 *            are to be harvested
	 */
	 ...
	 * </p>
	 *
	 * @param relayedCandidateSocket
	 *            the <tt>RelayedCandidateDatagramSocket</tt> which notifies
	 *            this instance and which requests that the associated TURN
	...
	protected boolean completedResolvingCandidate(Request request, Response response) {
		 ...
		 */
		if ((response == null)
				|| (!response.isSuccessResponse() && (request.getMessageType() == Message.ALLOCATE_REQUEST))) {
			try {
	...
	private void createRelayedCandidate(Response response) {
		...
		if (attribute instanceof XorRelayedAddressAttribute) {
			TransportAddress relayedAddress = ((XorRelayedAddressAttribute) attribute)
	 ...
	 *            <tt>RelayedCandidate</tt> instance
	 * @param mappedAddress
	 *            the mapped <tt>TransportAddress</tt> reported by the TURN
	 *            server with the delivery of the relayed
	 *            <tt>transportAddress</tt> to be represented by the new
	 *            <tt>RelayedCandidate</tt> instance
	 ...
	 *         {@link #harvester}
	 */
	protected RelayedCandidate createRelayedCandidate(TransportAddress transportAddress,
	...
	protected Request createRequestToRetry(Request request) {
		...
		case Message.ALLOCATE_REQUEST: {
			...
			int requestedTransport = (requestedTransportAttribute == null)
		...
		case Message.CHANNELBIND_REQUEST: {
			...
			Request retryChannelBindRequest = MessageFactory.createChannelBindRequest(channelNumber, peerAddress,
					retryTransactionID);
		...
		case Message.CREATEPERMISSION_REQUEST: {
			...
			TransportAddress peerAddress = peerAddressAttribute.getAddress(request.getTransactionID());
			byte[] retryTransactionID = TransactionID.createNewTransactionID().getBytes();
			Request retryCreatePermissionRequest = MessageFactory.createCreatePermissionRequest(peerAddress,
					retryTransactionID);
	 ...
	 *         {@link TurnCandidateHarvester#stunServer} in order to start
	...
	protected Request createRequestToStartResolvingCandidate() {
		if (requestToStartResolvingCandidate == null) {
			requestToStartResolvingCandidate = MessageFactory.createAllocateRequest(
					(byte) 17 /* User Datagram Protocol */, false);
	...
	protected boolean processErrorOrFailure(Response response, Request request, TransactionID transactionID) {
		...
		if ((applicationData instanceof RelayedCandidateDatagramSocket)
				&& ((RelayedCandidateDatagramSocket) applicationData).processErrorOrFailure(response, request))
			return true;
	...
	protected void processSuccess(Response response, Request request, TransactionID transactionID) {
		...
		case Message.ALLOCATE_RESPONSE:
			...
			lifetimeAttribute = (LifetimeAttribute) response.getAttribute(Attribute.LIFETIME);
		...
		if (lifetime >= 0) {
			setSendKeepAliveMessageInterval(/* milliseconds */ 1000L * lifetime);
		}
	 ...
	 *
	 * @param relayedCandidateDatagramSocket
	 *            the <tt>RelayedCandidateDatagramSocket</tt> which sends the
	 *            specified <tt>Request</tt> and which is to be notified of the
	 *            result
	 * @param request
	 *            the <tt>Request</tt> to be sent to the TURN server associated
	 *            with this <tt>TurnCandidateHarvest</tt>
	 * @return an array of <tt>byte</tt>s which represents the ID of the
	 *         transaction with which the specified <tt>Request</tt> has been
	 *         sent to the TURN server
	 * @throws StunException
	 *             if anything goes wrong while sending the specified
	 *             <tt>Request</tt>
	 */
	public byte[] sendRequest(RelayedCandidateDatagramSocket relayedCandidateDatagramSocket, Request request)
			throws StunException {
