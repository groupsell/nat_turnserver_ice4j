...
public class GoogleTurnCandidateHarvest extends StunCandidateHarvest {
	 ...
	 *            server.
	 */
	public GoogleTurnCandidateHarvest(GoogleTurnCandidateHarvester harvester, HostCandidate hostCandidate,
			String password) {
	...
	private void createRelayedCandidate(Response response) {
		Attribute attribute = response.getAttribute(Attribute.MAPPED_ADDRESS);
		...
		if (attribute != null) {
			...
			if (harvester.stunServer.getTransport() == Transport.TCP) {
				relayedAddress = new TransportAddress(relayedAddress.getAddress(), harvester.stunServer.getPort(),
						// relayedAddress.getPort() - 1,
						Transport.TCP);
			}
			GoogleRelayedCandidate relayedCandidate = createRelayedCandidate(relayedAddress,
	 ...
	 *         {@link #harvester}
	 */
	protected GoogleRelayedCandidate createRelayedCandidate(TransportAddress transportAddress,
			TransportAddress mappedAddress) {
		GoogleRelayedCandidate candidate = new GoogleRelayedCandidate(transportAddress, this, mappedAddress,
				harvester.getShortTermCredentialUsername(), this.password);
	...
	protected Request createRequestToStartResolvingCandidate() {
		if (requestToStartResolvingCandidate == null) {
			requestToStartResolvingCandidate = MessageFactory
					.createGoogleAllocateRequest(harvester.getShortTermCredentialUsername());
	...
	protected boolean processErrorOrFailure(Response response, Request request, TransactionID transactionID) {
		...
		if ((applicationData instanceof GoogleRelayedCandidateDatagramSocket)
			...
			return true;
		else if ((applicationData instanceof GoogleRelayedCandidateDatagramSocket)
				&& ((RelayedCandidateDatagramSocket) applicationData).processErrorOrFailure(response, request))
			return true;
