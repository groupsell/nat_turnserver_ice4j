...
public class IceSdpUtils {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>IceSdpUtils</tt> class and its
	 ...
	 *
	 * @param sDes
	 *            the session description where we'd like to set a user fragment
	 *            and a password.
	 * @param uFrag
	 ...
	 *             if the either of the parameters is null
	 */
	 ...
	 * candidates into m= lines and c= lines. This method uses media level c=
	 * lines. They override session level making them pointless. This shouldn't
	 * be causing problems, but for good taste, make sure you don't include
	 * session level ones and avoid duplication.
	 *
	 * @param mediaDescription
	 *            the media descriptions that we'd like to add candidates to.
	 * @param iceMediaStream
	 *            the media stream where we should extract candidates from.
	 */
	...
	public static void initMediaDescription(MediaDescription mediaDescription, IceMediaStream iceMediaStream) {
		try {
			...
			mediaDescription
					.setConnection(sdpFactory.createConnection("IN", defaultAddress.getHostAddress(), addressFamily));
	 ...
	 *
	 * @param sDes
	 *            the {@link SessionDescription} that we'd like to setup as per
	 *            the specified agent.
	 * @param agent
	 *            the {@link Agent} that we need to use when initializing the
	 *            session description.
	 *
	 * @throws IllegalArgumentException
	 *             Obviously, if there's a problem with the arguments ... duh!
	 */
	...
	public static void initSessionDescription(SessionDescription sDes, Agent agent) throws IllegalArgumentException {
		...
		try {
			...
			TransportAddress defaultAddress = agent.getStreams().get(0).getComponent(Component.RTP)
			...
			if (o == null || "user".equals(o.getUsername())) {
				...
				// default origin that has "user" as the user name so we use
				// this
			...
			for (IceMediaStream stream : streams) {
				MediaDescription mLine = sdpFactory.createMediaDescription(stream.getName(), 0, // default
																								// port
																								// comes
																								// later
	 ...
	 *
	 * @param localCandidates
	 *            the list of {@link LocalCandidate}s that we'd like to generate
	 *            the update for.
	 *
