...
class ConnectivityCheckServer implements RequestListener, CredentialsAuthority {
	/**
	 * The <tt>Logger</tt> used by the <tt>ConnectivityCheckServer</tt> class
	 * and its instances for logging output. Note that this shouldn't be used
	 * directly by instances of {@link ConnectivityCheckServer}, because it
	 * doesn't take into account the per-instance log level. Instances should
	...
	/**
	 * Handles the {@link Request} delivered in <tt>evt</tt> by possibly queuing
	 * a triggered check and sending a success or an error response depending on
	 * how processing goes.
	 *
	 * @param evt
	 *            the {@link StunMessageEvent} containing the {@link Request}
	 *            that we need to process.
	 *
	 * @throws IllegalArgumentException
	 *             if the request is malformed and the stack needs to reply with
	...
	public void processRequest(StunMessageEvent evt) throws IllegalArgumentException {
		...
		UsernameAttribute uname = (UsernameAttribute) request.getAttribute(Attribute.USERNAME);

		if (uname == null || !checkLocalUserName(new String(uname.getUsername()))) {
			return;
		}

		// detect role conflicts
		if ((parentAgent.isControlling() && request.containsAttribute(Attribute.ICE_CONTROLLING))
				|| (!parentAgent.isControlling() && request.containsAttribute(Attribute.ICE_CONTROLLED))) {
			if (!repairRoleConflict(evt))
				return;
		}

		long priority = 0;
		boolean useCandidate = request.containsAttribute(Attribute.USE_CANDIDATE);
		...
		parentAgent.incomingCheckReceived(evt.getRemoteAddress(), evt.getLocalAddress(), priority, remoteUfrag,
				localUFrag, useCandidate);
		...
		try {
			stunStack.sendResponse(evt.getTransactionID().getBytes(), response, evt.getLocalAddress(),
	 ...
	 *
	 * @param request
	 *            the {@link Request} whose priority we'd like to obtain.
	 *
	 * @return the value of the {@link PriorityAttribute} in <tt>request</tt> if
	 *         there is one
	 *
	 * @throws IllegalArgumentException
	 *             if the request does not contain a PRIORITY attribute and the
	 *             stack needs to respond with a 400 Bad Request
	 *             {@link Response}.
	 */
	private long extractPriority(Request request) throws IllegalArgumentException {
		...
		PriorityAttribute priorityAttr = (PriorityAttribute) request.getAttribute(Attribute.PRIORITY);
		...
		if (priorityAttr == null) {
			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE, "Received a connectivity check with" + "no PRIORITY attribute. Discarding.");
			}
	 ...
	 *
	 * @param evt
	 *            the {@link StunMessageEvent} containing the
	 *            <tt>ICE-CONTROLLING</tt> or <tt>ICE-CONTROLLED</tt> attribute
	 *            that allowed us to detect the role conflict.
	 *
	...
	private boolean repairRoleConflict(StunMessageEvent evt) {
		...
		if (parentAgent.isControlling() && req.containsAttribute(Attribute.ICE_CONTROLLING)) {
			IceControllingAttribute controlling = (IceControllingAttribute) req.getAttribute(Attribute.ICE_CONTROLLING);
			...
			// role.
				...
				logger.finer("Switching to controlled because theirTieBreaker=" + theirTieBreaker
						+ " and ourTieBreaker=" + ourTieBreaker);
		...
		// attribute is present in the request:
			...
			IceControlledAttribute controlled = (IceControlledAttribute) req.getAttribute(Attribute.ICE_CONTROLLED);
			...
			if (compareUnsignedLong(ourTieBreaker, theirTieBreaker) >= 0) {
				logger.finer("Switching to controlling because theirTieBreaker=" + theirTieBreaker
						+ " and ourTieBreaker=" + ourTieBreaker);
	...
	/**
	 * Verifies whether <tt>username</tt> is currently known to this server and
	 * returns <tt>true</tt> if so. Returns <tt>false</tt> otherwise.
	 *
	 * @param username
	 *            the user name whose validity we'd like to check.
	 *
	 ...
	 *
	 * @param username
	 *            the local ufrag that we should return a password for.
	 *
	 ...
	 *
	 * @param username
	 *            the remote ufrag that we should return a password for.
	 * @param media
	 *            the media name that we want to get remote key.
	 *
