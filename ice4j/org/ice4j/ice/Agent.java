...
public class Agent {
	 ...
	 * {@link IceProcessingState#COMPLETED} into
	 * {@link IceProcessingState#TERMINATED}.
	 */
	 ...
	 */
	private static final java.util.logging.Logger classLogger = java.util.logging.Logger
			.getLogger(Agent.class.getName());

	/**
	 * The name of the {@link PropertyChangeEvent} that we use to deliver events
	 * on changes in the state of ICE processing in this agent.
	 */
	public static final String PROPERTY_ICE_PROCESSING_STATE = "IceProcessingState";

	/**
	 * The LinkedHashMap used to store the media streams This map preserves the
	 * insertion order of the media streams.
	 */
	 ...
	 * addresses. This list is stored only if and while connectivity checks are
	 * not running. Once they start, we are able to determine whether the
	 * addresses in here are actually peer-reflexive or not, and schedule the
	 * necessary triggered checks.
	 */
	 ...
	 * address then we should add it to the list of candidates. Otherwise we
	 * should wait for the remote party to send their media description before
	...
	/**
	 * Contains {@link PropertyChangeListener}s registered with this
	 * {@link Agent} and following its changes of state.
	 */
	 ...
	 * harvest. We may use it to throw an exception at some point if it's ever a
	 * problem.
	 */
	 ...
	 * 
	 * @param loggingLevel
	 *            the logging level to be used by the agent and all of its
	 *            components.
	 */
	...
	public Agent(Level loggingLevel, String ufragPrefix) {
		...
		password = ensureIceAttributeLength(new BigInteger(128, random).toString(32), /* min */ 22, /* max */ 256);
	 ...
	 *
	 * @param stream
	 *            the {@link IceMediaStream} that the new {@link Component}
	 *            should belong to.
	 * @param transport
	 *            the transport protocol used by the component
	 * @param preferredPort
	 *            the port number that should be tried first when binding local
	 *            <tt>Candidate</tt> sockets for this <tt>Component</tt>.
	 * @param minPort
	 *            the port number where we should first try to bind before
	 *            moving to the next one (i.e. <tt>minPort + 1</tt>)
	 * @param maxPort
	 *            the maximum port number where we should try binding before
	 ...
	 *             if either <tt>minPort</tt> or <tt>maxPort</tt> is not a valid
	 *             port number or if <tt>minPort &gt;
	 * maxPort</tt>, or if <tt>transport</tt> is not currently supported.
	 * @throws IOException
	 *             if an error occurs while the underlying resolver lib is using
	 *             sockets.
	 * @throws BindException
	 *             if we couldn't find a free port between <tt>minPort</tt> and
	 *             <tt>maxPort</tt> before reaching the maximum allowed number
	 *             of retries.
	 */
	public Component createComponent(IceMediaStream stream, Transport transport, int preferredPort, int minPort,
			int maxPort) throws IllegalArgumentException, IOException, BindException {
	 ...
	 *            the keep-alive strategy, which dictates which candidates pairs
	 *            are going to be kept alive.
	 *
	 * @return the newly created {@link Component} and with a list containing
	 *         all and only local candidates.
	 *
	 ...
	 */
	public Component createComponent(IceMediaStream stream, Transport transport, int preferredPort, int minPort,
			int maxPort, KeepAliveStrategy keepAliveStrategy)
			throws IllegalArgumentException, IOException, BindException {
		return createComponent(stream, transport, preferredPort, minPort, maxPort, keepAliveStrategy, true);
	}
	 ...
	 *            the keep-alive strategy, which dictates which candidates pairs
	 *            are going to be kept alive.
	 * @param useComponentSocket
	 *            whether to use the component socket mode (in which case the
	 *            socket is available through the {@link Component} directly),
	 *            or not (in which case the socket is available through the
	 *            selected {@link CandidatePair}).
	 *
	 * @return the newly created {@link Component} and with a list containing
	 *         all and only local candidates.
	 *
	 ...
	 */
	public Component createComponent(IceMediaStream stream, Transport transport, int preferredPort, int minPort,
			int maxPort, KeepAliveStrategy keepAliveStrategy, boolean useComponentSocket)
			throws IllegalArgumentException, IOException, BindException {
		if (transport != Transport.UDP) {
			throw new IllegalArgumentException(
					"This implementation does not currently support transport: " + transport);
		}
		 ...
		 * stopped the ConnectivityCheckServer. If the ConnectivityCheckServer
		 * is not started after we've made the gathered LocalCandidates
		 * available to the caller, the caller may send them and a connectivity
		 * check may arrive from the remote Agent.
		 */
	 ...
	 *
	 * @param local
	 *            the {@code LocalCandidate} to initialize the new instance with
	 * @param remote
	 *            the {@code RemoteCandidate} to initialize the new instance
	 *            with
	 ...
	 * 
	 * @param component
	 *            the <tt>Component</tt> that we'd like to gather candidates
	 *            for.
	 * @param preferredPort
	 *            the port number that should be tried first when binding local
	 *            <tt>Candidate</tt> sockets for this <tt>Component</tt>.
	 ...
	 * maxPort</tt>.
	 * @throws IOException
	 *             if an error occurs while the underlying resolver lib is
	 *             gathering candidates and we end up without even a single one.
	 */
	private void gatherCandidates(Component component, int preferredPort, int minPort, int maxPort)
			throws IllegalArgumentException, IOException {
		logger.info(
				"Gathering candidates for component " + component.toShortString() + ". Local ufrag " + getLocalUfrag());
	 ...
	 *            the callback that will be notified for all newly discovered
	 *            candidates.
	 *
	 * @throws IllegalStateException
	 *             if we try calling this method without being in a trickling
	 *             state.
	 */
	 ...
	 * process has started is important, for example, when determining whether a
	 * remote address we've just discovered is peer reflexive or not. If ICE has
	 * started and we don't know about the address then we should add it to the
	 * list of candidates. Otherwise we should hold to it until it does and
	 * check later.
	 * <p>
	 ...
	 *         current state) and <tt>false</tt> otherwise.
	 */
	public boolean isStarted() {
		return state != IceProcessingState.WAITING && state != IceProcessingState.COMPLETED
	 ...
	 *
	 * @return <tt>true</tt> if ICE processing is in the
	 *         {@link IceProcessingState#FAILED},
	 *         {@link IceProcessingState#COMPLETED} or
	 *         {@link IceProcessingState#TERMINATED} and <tt>false</tt>
	 *         otherwise.
	 */
	 ...
	 *
	 * @param oldState
	 *            the {@link IceProcessingState} we had before the change
	 * @param newState
	 *            the {@link IceProcessingState} we had after the change
	 */
	 ...
	 *
	 * @param newState
	 *            the new state of ICE processing for this <tt>Agent</tt>.
	 * @return <tt>true</tt> iff the state of this <tt>Agent</tt> changed as a
	 *         result of this call.
	 */
	...
	/**
	 * Creates, initializes and orders the list of candidate pairs that would be
	 * used for the connectivity checks for all components in this stream.
	 */
	 ...
	 *
	 * @param component
	 *            the component whose candidate foundations we'd like to compute
	 *            and assign.
	 */
	 ...
	 *
	 * @param harvester
	 *            a <tt>CandidateHarvester</tt> that this agent should use when
	 ...
	 * provided by the peer. For example, consider the case where agent L is the
	 * offerer, and agent R is the answerer. Agent L included a username
	 * fragment of LFRAG for its candidates, and a password of LPASS. Agent R
	 * provided a username fragment of RFRAG and a password of RPASS. A
	 * connectivity check from L to R (and its response of course) utilize the
	 * username RFRAG:LFRAG and a password of RPASS. A connectivity check from R
	 * to L (and its response) utilize the username LFRAG:RFRAG and a password
	 * of LPASS.
	 *
	 * @param media
	 *            media name that we want to generate local username for.
	 ...
	 *
	 * @param candidate1
	 *            The first candidate of a candidatePair.
	 * @param candidate2
	 *            The second candidate of a candidatePair.
	 * @return a user name that a peer <tt>Agent</tt> would use in connectivity
	 *         check for outgoing Binding Requests.
	 */
	 ...
	 * 
	 * @param stunStack
	 *            the stunStack to be used by this Agent.
	 * 
	 */
	 ...
	 *
	 * @param isControlling
	 *            <tt>true</tt> if this is to be the controlling <tt>Agent</tt>
	 ...
	 *
	 * @param stream
	 *            the <tt>Component</tt> we'd like to remove and free.
	 */
	 ...
	 * exchange.
	 *
	 * @return <tt>true</tt> if this is to be the controlling <tt>Agent</tt> and
	 ...
	 *
	 * @param localAddress
	 *            the {@link TransportAddress} we are looking for.
	 *
	 * @return the local <tt>LocalCandidate</tt> with the specified
	 *         <tt>localAddress</tt> if it belongs to any of this
	 ...
	 *         <tt>remoteAddress</tt> if it belongs to any of this
	 *         {@link Agent}'s streams or <tt>null</tt> if it doesn't.
	 */
	 ...
	 *
	 * @param localAddress
	 *            the local {@link TransportAddress} of the pair we are looking
	 *            for.
	 * @param remoteAddress
	 *            the remote {@link TransportAddress} of the pair we are looking
	 *            for.
	 *
	 * @return the {@link CandidatePair} with the specified remote and local
	 *         addresses or <tt>null</tt> if neither of the {@link CheckList}s
	 *         in this {@link Agent}'s streams contain such a pair.
	 */
	 ...
	 * will cause us to schedule a triggered check for the corresponding remote
	 * candidate and potentially to the discovery of a PEER-REFLEXIVE candidate.
	 *
	 * @param remoteAddress
	 *            the address that we've just seen, and that is potentially a
	 *            peer-reflexive address.
	 * @param localAddress
	 *            the address that we were contacted on.
	 * @param priority
	 *            the priority that the remote party assigned to
	 * @param remoteUFrag
	 *            the user fragment that we should be using when and if we
	 *            decide to send a check to <tt>remoteAddress</tt>.
	 * @param localUFrag
	 *            local user fragment
	 * @param useCandidate
	 *            indicates whether the incoming check
	 *            {@link org.ice4j.message.Request} contained the USE-CANDIDATE
	 *            ICE attribute.
	 */
	protected void incomingCheckReceived(TransportAddress remoteAddress, TransportAddress localAddress, long priority,
			String remoteUFrag, String localUFrag, boolean useCandidate) {
		String ufrag = null;
		...
		RemoteCandidate remoteCandidate = new RemoteCandidate(remoteAddress, parentComponent,
				CandidateType.PEER_REFLEXIVE_CANDIDATE, foundationsRegistry.obtainFoundationForPeerReflexiveCandidate(),
				priority,
		...
		synchronized (startLock) {
			...
			{
				if (logger.isLoggable(Level.FINE)) {
					logger.debug("Received check from " + triggeredPair.toShortString() + " triggered a check. "
	 ...
	 *
	 * @param triggerPair
	 *            the pair containing the local and remote candidate that we'd
	 *            need to trigger a check for.
	 */
	private void triggerCheck(CandidatePair triggerPair) {
		 ...
		 * elsewhere as soon as and only if the CheckList changes from frozen to
		 * unfrozen. Since CheckList#scheduleTriggeredCheck will change
	 ...
	 *
	 * @param validPair
	 *            the {@link CandidatePair} we'd like to validate.
	 */
	 ...
	 *
	 * @param pair
	 *            the {@link CandidatePair} that we'd like to nominate and that
	 *            we'd like to schedule a triggered check for.
	 *
	 * @throws IllegalStateException
	 *             if this <tt>Agent</tt> is not a controlling agent and can
	 *             therefore not nominate pairs.
	 *
	 ...
	 *
	 * @param strategy
	 *            the strategy that we'd like to use for nominating valid
	 *            {@link CandidatePair}s.
	 */
	 ...
	 *
	 * @param nominatedPair
	 *            the {@link CandidatePair} whose nomination has just been
	 *            confirmed.
	 */
	...
	/**
	 * After updating check list states as a result of an incoming response or a
	 * timeout event the method goes through all check lists and tries to assess
	...
	protected void checkListStatesUpdated() {
		...
		if (stunKeepAliveThread == null && !StackProperties.getBoolean(StackProperties.NO_KEEP_ALIVES, false)) {
	 ...
	 * of local candidates that were selected as well as the server that were
	 * used (if any) to obtain them.
	 */
	...
	/**
	 * Lets the application specify a custom value for the <tt>Ta</tt> timer so
	 * that we don't calculate one.
	 *
	 * @param taValue
	 *            the value of the <tt>Ta</tt> timer that the application would
	 *            like us to use rather than calculate one.
	 */
	 ...
	 * performing connectivity checks (Section 7), an agent sends STUN and TURN
	 * transactions. These transactions are paced at a rate of one every
	 * <tt>Ta</tt> milliseconds.
	 * <p>
	 ...
	 *
	 * @return the value of the <tt>Ta</tt> pace timer according to the number
	 *         and type of {@link IceMediaStream}s this agent will be using or a
	 *         pre-configured value if the application has set one.
	 */
	protected long calculateTa() {
		 ...
		 *
		 * 1 Ta = MAX (20ms, ------------------- ) k ---- \ 1 > ------ / Ta_i
		 * ---- i=1
		 *
		 * In this implementation we assume equal values of stun_packet_size and
		 * rtp_packet_size. rtp_ptime is also assumed to be 20ms. One day we
		 * should probably let the application modify them. Until then however
		 * the above formula would always be equal to. 1 Ta = MAX (20ms, -------
		 * ) k --- 20 which gives us Ta = MAX (20ms, 20/k) which is always 20.
		 */
		return 20;
	}
	 ...
	 * transactions, while harvesting addresses (not to confuse with the RTO for
	 * the STUN transactions used in connectivity checks).
	 *
	...
	protected long calculateStunHarvestRTO() {
		/*
		 * RFC 5245 says: RTO = MAX (100ms, Ta * (number of pairs)) where the
		 * number of pairs refers to the number of pairs of candidates with STUN
		 * or TURN servers.
		 *
	...
	protected long calculateStunConnCheckRTO() {
		/*
		 * RFC 5245 says: For connectivity checks, RTO SHOULD be configurable
	...
	private void scheduleStunKeepAlive() {
		if (stunKeepAliveThread == null) {
			stunKeepAliveThread = new Thread() {
				@Override
				public void run() {
	 ...
	 *            the state that we'd like processing to terminate with i.e.
	 *            either {@link IceProcessingState#TERMINATED} or
	 *            {@link IceProcessingState#FAILED}
	 */
	 ...
	 *
	 * @param s
	 *            the <tt>String</tt> to add or remove characters to or from in
	 *            case its length is less than <tt>min</tt> or greater than
	 *            <tt>max</tt>
	 * @param min
	 ...
	 *            <tt>String</tt>
	 * @return <tt>s</tt> if its length is greater than or equal to <tt>min</tt>
	 *         and less than or equal to <tt>max</tt>; a new <tt>String</tt>
	 *         which is equal to <tt>s</tt> with prepended ICE characters if the
	 *         length of <tt>s</tt> is less than <tt>min</tt>; a new
	 *         <tt>String</tt> which is composed of the first <tt>max</tt>
	 *         characters of <tt>s</tt> if the length of <tt>s</tt> is greater
	 *         than <tt>max</tt>
	 * @throws IllegalArgumentException
	 *             if <tt>min</tt> is negative or <tt>max</tt> is less than
	 *             <tt>min</tt>
	 * @throws NullPointerException
	 *             if <tt>s</tt> is equal to <tt>null</tt>
	 */
	 ...
	 *
	 * @throws Throwable
	 *             if anything goes wrong and the finalization of this instance
	 *             is to be halted
	 * @see #free()
	 */
	@Override
	 ...
	 *
	 * @param generation
	 *            the current generation of this ICE Agent
	 */
	...
	private void runInStunKeepAliveThread() {
		long consentFreshnessInterval = Long.getLong(StackProperties.CONSENT_FRESHNESS_INTERVAL,
		...
		int consentFreshnessMaxRetransmissions = Integer.getInteger(
				StackProperties.CONSENT_FRESHNESS_MAX_RETRANSMISSIONS, DEFAULT_CONSENT_FRESHNESS_MAX_RETRANSMISSIONS);
	 ...
	 *         harvesting when components are being added and <tt>false</tt>
	 *         otherwise.
	 */
	...
	/**
	 * Determines whether this agent will trickle candidates rather than gather
	 * them synchronously while components are being added. When trickling is
	 * turned on, the agent will only gather host addresses for newly added
	 * components. When trickling is off, all harvesting for a specific
	 * component will be executed when that component is being added.
	 *
	 * @param trickle
	 *            <tt>false</tt> if this agent is configured to perform all
	 *            harvesting when components are being added and <tt>false</tt>
	 *            otherwise.
	 */
	 ...
	 *
	 * @param harvesterName
	 *            the class name of the harvester for which the number of
	 *            completed harvests is to be returned
	 * @return the number of harvests that the harvester with the specified
	 *         <tt>harvesterName</tt> has completed so far if such a harvester
	 *         exists and has completed at least one harvest; otherwise, zero
	 */
	 ...
	 * perform consent freshness.
	 *
	 * @param performConsentFreshness
	 *            <tt>true</tt> if this <tt>Agent</tt> is to perform consent
	 *            freshness; otherwise, <tt>false</tt>
	 */
	...
	/**
	 * Sets the flag which indicates whether the dynamic host harvester will be
	 * used or not by this <tt>Agent</tt>.
	 * 
	 * @param useHostHarvester
	 *            the value to set.
	 */
	 ...
	 * all peers for media streams using those candidates, the agent SHOULD wait
	 * an additional three seconds, and then it MAY cease responding to checks
	...
	private class TerminationThread extends Thread {
		...
		/**
		 * Waits for a period of three seconds (or whatever termination interval
		 * the user has specified) and then moves this <tt>Agent</tt> into the
		 * terminated state and frees all non-nominated candidates.
		 */
		@Override
