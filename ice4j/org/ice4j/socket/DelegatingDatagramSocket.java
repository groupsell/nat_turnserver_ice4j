...
public class DelegatingDatagramSocket extends DatagramSocket {
	 ...
	 * "java" DatagramSocket. This way external applications can define the type
	 * of DatagramSocket to use. If "null", then the classical "java"
	 * DatagramSocket is used. Otherwise the factory generates custom
	 * DatagramSockets.
	 */
	 ...
	 * DelegatingDatagramSocket will automatically use a socket from the factory
	 ...
	 *
	 * @param factory
	 *            The factory assigned to generates the new DatagramSocket for
	 *            each new DelegatingDatagramSocket which do not use a delegate
	 *            socket.
	 */
	...
	/**
	 * Specifies a default receive buffer size for the underlying sockets. This
	 * function must be called before using the DelegatingDatagramSocket
	...
	/**
	 * Determines whether a packet should be logged, given the number of sent or
	 * received packets.
	 *
	 * @param numOfPacket
	...
	static boolean logNonStun(long numOfPacket) {
		return (numOfPacket == 1) || (numOfPacket == 300) || (numOfPacket == 500) || (numOfPacket == 1000)
				|| ((numOfPacket % 5000) == 0);
	}
	 ...
	 * to any available port on the local host machine. The socket will be bound
	 * to the wildcard address, an IP address chosen by the kernel.
	 *
	 ...
	 *
	 * @param delegate
	 *            the <tt>DatagramSocket</tt> to which the new instance is to
	 *            delegate
	 * @throws SocketException
	...
	/**
	 * Initializes a new <tt>DelegatingDatagramSocket</tt> instance and binds it
	 * to the specified port on the local host machine. The socket will be bound
	 * to the wildcard address, an IP address chosen by the kernel.
	 *
	 * @param port
	 *            the port to bind the new socket to
	 * @throws SocketException
	 *             if the socket could not be opened, or the socket could not
	 *             bind to the specified local port
	 ...
	 *
	 * @param port
	 *            the local port to bind the new socket to
	 * @param laddr
	 *            the local address to bind the new socket to
	 * @throws SocketException
	 *             if the socket could not be opened, or the socket could not
	 *             bind to the specified local port
	 ...
	 *
	 * @param bindaddr
	 *            local socket address to bind, or <tt>null</tt> for an unbound
	 *            socket
	 * @throws SocketException
	 *             if the socket could not be opened, or the socket could not
	 *             bind to the specified local port
	 ...
	 *            delegate.
	 * @param address
	 *            The local socket address to bind, or <tt>null</tt> for an
	 *            unbound socket.
	 *
	 * @throws SocketException
	 *             if the socket could not be opened, or the socket could not
	 ...
	 *             if a security manager exists and its <tt>checkListen</tt>
	 *             method doesn't allow the operation
	 * @throws IllegalArgumentException
	 *             if <tt>addr</tt> is a <tt>SocketAddress</tt> subclass not
	 ...
	 * @throws IllegalArgumentException
	 *             if the address is <tt>null</tt>, or the port is out of range
	 * @throws SecurityException
	 *             if the caller is not allowed to send datagrams to and receive
	 *             datagrams from the address and port
	 ...
	 * @throws IllegalArgumentException
	 *             if <tt>addr</tt> is <tt>null</tt> or a <tt>SocketAddress</tt>
	...
	/**
	 * Retrieves setting for <tt>SO_TIMEOUT</tt>. Zero returned implies that the
		...
		if (delegate == null) {
			...
			// network stack.
				...
				if (((ex instanceof NoRouteToHostException)
						...
						&& (tmpAddr instanceof Inet6Address) && (tmpAddr.isLinkLocalAddress())) {
					Inet6Address newAddr = Inet6Address.getByAddress("", tmpAddr.getAddress(),
							((Inet6Address) super.getLocalAddress()).getScopeId());
	 ...
	 *             UDP error
	 * @throws IllegalArgumentException
	 *             if the value is zero or is negative
	 ...
	 *
	 * @param on
	 *            whether to enable or disable the <tt>SO_REUSEADDR</tt> socket
	 *            option
	 * @throws SocketException
	 ...
	 *
	 * @param tc
	 *            an <tt>int</tt> value for the bitset
	 * @throws SocketException
