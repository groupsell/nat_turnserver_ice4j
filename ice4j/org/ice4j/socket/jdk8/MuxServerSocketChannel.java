...
public class MuxServerSocketChannel extends DelegatingServerSocketChannel<MuxingServerSocketChannel> {
	 ...
	 * by {@link #accept()}.
	 */
	private final Queue<Timestamped<SocketChannel>> acceptQ = new LinkedList<>();
	 ...
	 *
	 * @param delegate
	 *            the {@code MuxingServerSocketChannel} which is actually open
	 *            and bound to a listening endpoint and accepts
	 *            {@code SocketChannel}s to be filtered by associated
	 *            {@code MuxServerSocketChannel}s
	 * @param filter
	 *            the {@code DatagramPacketFilter} which is to demultiplex
	...
	void closeAbandonedSocketChannels(long now) {
		synchronized (syncRoot) {
			...
			if (!chs.isEmpty()) {
				for (Iterator<Timestamped<SocketChannel>> i = chs.iterator(); i.hasNext();) {
					Timestamped<SocketChannel> ch = i.next();

					if (now - ch.timestamp >= MuxingServerSocketChannel.SOCKET_CHANNEL_READ_TIMEOUT) {
						i.remove();
	 ...
	 *
	 * @param p
	 *            the {@code DatagramPacket} which has been read from
	 *            {@code channel} and which is to be analyzed by the
	 *            {@code filter} of this instance
	 * @param channel
	 *            the {@code SocketChannel} from which {@code p} has been read
	 *            and which is to be queued for acceptance through this instance
	 *            if the {@code filter} accepts {@code p}
	 * @return {@code true} if the {@code filter} of this instance accepts
	 *         {@code p} and {@code channel} was queued for acceptance through
	 *         this instance; otherwise, {@code false}
	 */
	protected boolean filterAccept(DatagramPacket p, SocketChannel channel) {
		return filter.accept(p) && qAccept(new PreReadSocketChannel(p, channel));
	}
	 ...
	 *
	 * @param channel
	 *            the {@code SocketChannel} to queue for acceptance through this
	 *            instance
	...
	boolean qAccept(SocketChannel channel) {
		...
		synchronized (syncRoot) {
			if (acceptQ.offer(new Timestamped<>(channel, System.currentTimeMillis()))) {
				syncRoot.notifyAll();
				b = true;
			} else {
				b = false;
