...
class MuxingServerSocketChannel extends DelegatingServerSocketChannel<ServerSocketChannel> {
	 ...
	 */
	private static final int SELECTOR_SELECT_TIMEOUT = MuxServerSocketChannelFactory.SOCKET_CHANNEL_READ_TIMEOUT;
	 ...
	 */
	static final int SOCKET_CHANNEL_READ_TIMEOUT = MuxServerSocketChannelFactory.SOCKET_CHANNEL_READ_TIMEOUT;
	 ...
	 * {@code MuxServerSocketChannel}s.
	 */
	private static final int SOCKET_CHANNEL_READ_CAPACITY = Math.max(
			GoogleTurnSSLCandidateHarvester.SSL_CLIENT_HANDSHAKE.length,
			Math.max(HttpDemuxFilter.REQUEST_METHOD_MAX_LENGTH + 1 /* SP */, HttpDemuxFilter.TLS_MIN_LENGTH));
	 ...
	 *
	 * @param channel
	 *            the {@code MuxingServerSocketChannel} to add to the (global)
	 *            list of existing {@code MuxingServerSocketChannel}s and to
			...
			while (i.hasNext()) {
				...
				if (aChannel.isOpen()) {
					...
					if (!matches && aLocalAddr instanceof InetSocketAddress && localAddr instanceof InetSocketAddress) {
						...
						matches = aLocalInetAddr.getAddress().equals(ANY_LOCAL_ADDRESS)
	 ...
	 *
	 * @param properties
	 *            a {@code Map} of the values to be assigned to properties of
	 *            the underlying {@link ServerSocketChannel} which is to
	 *            actually listen on the specified {@code endpoint}. If the new
	 *            instance is not the first to open and bind the specified
	 *            {@code endpoint}, the {@code properties} and their respective
	 *            values may not be used.
	 * @param endpoint
	 *            the IP and port the new instance is to bind to
	 * @param backlog
	 *            the requested maximum number of pending incoming connections
	 *            to be queued. If the new instance is not the first to open and
	 *            bind the specified {@code endpoint}, the value may not be
	 *            used.
	 * @param filter
	 *            the {@code DatagramPacketFilter} to demultiplex (i.e.
	 ...
	 *             if an I/O error occurs
	 */
	public static MuxServerSocketChannel openAndBind(Map<String, Object> properties, SocketAddress endpoint,
			int backlog, DatagramPacketFilter filter) throws IOException {
		...
		synchronized (muxingServerSocketChannels) {
			...
			if (muxingChannel == null) {
				ServerSocketChannel channel = MuxServerSocketChannelFactory.openAndBindServerSocketChannel(properties,
						endpoint, backlog);
	...
	private static void runInAcceptThread() {
		runInSelectorThread(/* syncRoot */ muxingServerSocketChannels, /* threadSupplier */ new Supplier<Thread>() {
			@Override
			public Thread get() {
		...
		}, /* selectionKeyOps */ SelectionKey.OP_ACCEPT, /* channels */ muxingServerSocketChannels,
	 ...
	 *
	 * @param syncRoot
	 *            the {@code Object} to synchronize the access to
	 *            {@code threadSupplier}, {@code selectorSupplier},
	 *            {@code channels}, and {@code predicate}. It should be notified
	 *            whenever the values supplied by {@code threadSupplier},
	 *            {@code selectorSupplier}, and {@code channels} change.
	 * @param threadSupplier
	 *            the {@link Supplier} which is to supply the {@code Thread} in
	 *            which the method is supposed to be running. If the returned
	 *            value differs from the {@code Thread} in which the method is
	 *            actually running (i.e. {@link Thread#currentThread()}, the
	 *            method returns. In other words, {@code threadSupplier} is one
	 *            of the ways to break out of the loop implemented by the
	 *            method. The {@code threadSupplier} is called on while
	 *            {@code syncRoot} is acquired.
	 * @param selectorSupplier
	 *            the {@code Supplier} which is to supply the {@code Selector}
	 *            on which the method is to await changes in the states of
	 *            {@code channels} in order to begin a subsequent loop
	 *            iteration. If the returned {@code Selector} is not open, the
	 *            method returns. In other words, {@code selectorSupplier} is
	 *            another way to break out of the loop implemented by the
	 *            method. The {@code selectorSupplier} is called on while
	 *            {@code syncRoot} is acquired.
	 * @param selectionKeyOps
	 *            the {@link SelectionKey} operation-set bits which identify the
	 *            states of {@code channels} whose changes trigger new loop
	 *            iterations
	 * @param channels
	 *            the (set of) {@code SelectableChannel}s on each of which
	 *            {@code predicate} is to be continually tested. A loop
	 *            iteration is triggered when at least one of {@code channels}
	 *            has a state identified by {@code selectionKeyOps} changes.
	 * @param predicate
	 *            the {@code Predicate} which is to be continually tested on
	 *            {@code channels}. A loop iteration is triggered when at least
	 *            one of {@code channels} has a state identified by
	 *            {@code selectionKeyOps} changes.
	 *            {@link BiPredicate#test(Object, Object)} is supplied with an
	 *            element of {@code channels} and its (automatically) associated
	 *            {@code SelectionKey} in the {@code Selector} returned by
	 *            {@code selectorSupplier}. The {@code SelectionKey} is provided
	 *            in case, for example, the implementation of {@code predicate}
	 *            chooses to associate additional state with the
	 *            {@code SelectableChannel} (through
	 *            {@link SelectionKey#attach(Object)}) available throughout the
	 *            whole loop.
	 * @param <T>
	 *            the element type of {@code channels}
	 */
	private static <T extends SelectableChannel> void runInSelectorThread(Object syncRoot,
			Supplier<Thread> threadSupplier, Supplier<Selector> selectorSupplier, int selectionKeyOps,
			Iterable<T> channels, BiPredicate<T, SelectionKey> predicate) {
		...
		final int selSelectTimeout = Math.min(SELECTOR_SELECT_TIMEOUT, SOCKET_CHANNEL_READ_TIMEOUT);
	 ...
	 *
	 * @param channel
	 *            the {@code MuxingServerSocketChannel} to schedule for
	 *            acceptance of incoming network connections in
	 *            {@code acceptThread}
	 * @throws IOException
	 *             if an I/O error occurs
	 */
	 ...
	 * this instance.
	 */
	private final List<MuxServerSocketChannel> muxServerSocketChannels = new ArrayList<>();
	 ...
	 *
	 * @param delegate
	 *            the {@code ServerSocketChannel} for which the new instance is
	 *            to provide listening endpoint sharing
	 * @throws IOException
	 *             if an I/O error occurs
	 */
	 ...
	 *
	 * @param channel
	 *            the {@code MuxServerSocketChannel} to add
	 */
	 ...
	 *
	 * @param filter
	 *            the {@code DatagramPacketFilter} to be used by the new
	 *            {@code MuxServerSocketChannel} instance to demultiplex
	 *            incoming network connections and packets
	 * @return a new {@code MuxServerSocketChannel} instance which delegates to
	 *         this instance and demultiplexes incoming network connections and
	...
	protected MuxServerSocketChannel createMuxServerSocketChannel(DatagramPacketFilter filter) {
		...
		synchronized (syncRoot) {
			for (Iterator<MuxServerSocketChannel> i = muxServerSocketChannels.iterator(); i.hasNext();) {
	 ...
	 *
	 * @param p
	 *            the {@code DatagramPacket} read from {@code channel} which is
	 *            to be analyzed by the {@code MuxServerSocketChannel}s created
	 *            by and delegating to this instance
	 * @param channel
	 *            the {@code SocketChannel} from which {@code p} was read and
	 *            which is to possibly be demultiplexed into a
	 ...
	 *
	 * @param channel
	 *            the {@code SocketChannel} to read from
	 * @param buf
	 *            the {@code ByteBuffer} to write into
	 * @return the number of {@code byte}s read from {@code channel} and written
	 *         into {@code buf} or {@code -1} if {@code channel} has reached the
	 *         end of its stream
	 */
	 ...
	 *
	 * @param channel
	 *            the added <tt>MuxServerSocketChannel</tt>
	 */
	...
	protected void runInReadThread() {
		runInSelectorThread(/* syncRoot */ syncRoot, /* threadSupplier */ new Supplier<Thread>() {
			@Override
			public Thread get() {
				return readThread;
			}
		...
		}, /* selectionKeyOps */ SelectionKey.OP_READ, /* channels */ readQ,
	 ...
	 *
	 * @param channel
	 *            the {@code SocketChannel} to queue for reading and
	 *            demultiplexing
	 */
	protected void scheduleRead(SocketChannel channel) {
		synchronized (syncRoot) {
			if (readThread == null) {
				...
				readThread.setName(MuxingServerSocketChannel.class.getName() + ".readThread");
				readThread.start();
			} else {
	 ...
	 *
	 * @param ch
	 *            the {@code SocketChannel} to read from and to classify/filter
	 *            into a {@code MuxServerSocketChannel} for acceptance
	 * @param sk
	 *            the {@code SelectionKey} associated with {@code ch} in the
	 *            {@code Selector} which awaits changes in the state(s) of
	 *            {@code ch}
