 ...
 */
class DelegatingSocketChannel<T extends SocketChannel> extends BaseDelegatingSocketChannel<T> implements SelChImpl {
	/**
	 ...
	 *
	 * @param delegate
	 *            the {@code SocketChannel} the new instance is to delegate (its
	 *            method calls) to
	 */
	public DelegatingSocketChannel(T delegate) {
		super(delegate);

		delegateAsSelChImpl = (delegate instanceof SelChImpl) ? (SelChImpl) delegate : null;
	}
