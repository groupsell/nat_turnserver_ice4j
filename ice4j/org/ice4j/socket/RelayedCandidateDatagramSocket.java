...
public class RelayedCandidateDatagramSocket extends DatagramSocket implements MessageEventHandler {
	 ...
	 * <tt>RelayedCandidateDatagramSocket</tt>s relays data send to it to peer
	 ...
	 *
	 * @param relayedCandidate
	 *            the <tt>RelayedCandidate</tt> which is to use the new instance
	 *            as the value of its <tt>socket</tt> property
	 * @param turnCandidateHarvest
	 *            the <tt>TurnCandidateHarvest</tt> which has harvested
	...
	public RelayedCandidateDatagramSocket(RelayedCandidate relayedCandidate, TurnCandidateHarvest turnCandidateHarvest)
		...
		DatagramSocket hostSocket = this.turnCandidateHarvest.hostCandidate.getCandidateIceSocketWrapper()
				.getUDPSocket();

		if (hostSocket instanceof MultiplexingDatagramSocket) {
			channelDataSocket = ((MultiplexingDatagramSocket) hostSocket)
					.getSocket(new TurnDatagramPacketFilter(this.turnCandidateHarvest.harvester.stunServer) {
						@Override
	 ...
	 *
	 * @param p
	 *            the <tt>DatagramPacket</tt> which is to be checked whether it
	 *            is accepted by <tt>channelDataSocket</tt>
	 * @return <tt>true</tt> if <tt>channelDataSocket</tt> accepts <tt>p</tt>
	 *         (i.e. <tt>channelDataSocket</tt> understands <tt>p</tt> and
	 *         <tt>p</tt> is meant to be received by
	 *         <tt>channelDataSocket</tt>); otherwise, <tt>false</tt>
	 */
	 ...
	 *            <tt>DatagramPacket</tt>s which is accepted by
	 *            <tt>channelDataSocket</tt>
	 * @return <tt>true</tt> if <tt>channelDataSocket</tt> accepts
	 *         <tt>DatagramPacket</tt>s which represent STUN messages with the
	 *         specified <tt>method</tt>; otherwise, <tt>false</tt>
	 */
	...
	public void close() {
		...
		turnCandidateHarvest.harvester.getStunStack()
				.removeIndicationListener(turnCandidateHarvest.hostCandidate.getTransportAddress(), this);
		...
		receiveChannelDataThread = new Thread() {
			...
			public void run() {
				...
				try {
					 ...
					 * RelayedCandidateDatagramSocket is not closed, then spawn
					 * a new receiveChannelDataThread.
					 */
					synchronized (packetsToReceive) {
	 ...
	 *         <tt>InetAddress</tt> representing any local address if either the
	 *         socket is not bound, or the security manager
	 *         <tt>checkConnect</tt> method does not allow the operation
	 ...
	 *
	 * @param e
	 *            a <tt>StunMessageEvent</tt> which encapsulates the received
	 *            STUN message
	 */
	public void handleMessageEvent(StunMessageEvent e) {
		 ...
		 */
		XorPeerAddressAttribute peerAddressAttribute = (XorPeerAddressAttribute) message
		...
		try {
			...
			/*
			 * The signature of the DatagramPacket constructor was changed in
			 * JDK 8 to not declare that it may throw a SocketException.
			 */
			if (t instanceof SocketException) {
				...
				/*
				 * Unfortunately, we cannot re-throw it. Anyway, it was unlikely
				 * to occur on JDK 7.
				 */
	 ...
	 *
	 * @param response
	 *            the <tt>Response</tt> which responds to <tt>request</tt>
	 * @param request
	 *            the <tt>Request</tt> sent by this instance to which
	 ...
	 *
	 * @param p
	 *            the <tt>DatagramPacket</tt> into which to place the incoming
	 *            data
	 * @throws IOException
			...
			do {
				...
				if (closed) {
					throw new SocketException(
							RelayedCandidateDatagramSocket.class.getSimpleName() + " has been closed.");
	...
	private void runInReceiveChannelDataThread() throws SocketException {
		...
		while (!closed) {
			...
			char channelNumber = (char) ((channelData[channelDataOffset++] << 8)
		...
		synchronized (packetsToSend) {
			while (!closed) {
				...
				for (int packetToSendIndex = 0; packetToSendIndex < packetToSendCount; packetToSendIndex++) {
					 ...
					 */
					int channelCount = channels.size();
					TransportAddress peerAddress = new TransportAddress(packetToSend.getAddress(),
					...
					if ((channelDataSocket != null) && !channel.getChannelDataIsPreferred()
							&& !connectivityCheckRecognizer.accept(packetToSend)) {
					...
					if (!forceBind && channel.isBound()) {
						...
						try {
							...
							if (logger.isLoggable(Level.INFO)) {
								logger.log(
										Level.INFO, "Failed to send through "
												+ RelayedCandidateDatagramSocket.class.getSimpleName() + " channel.",
										sex);
							}
						}
						break;
	 ...
	 *
	 * @param p
	 *            the <tt>DatagramPacket</tt> to be sent
	 * @throws IOException
	 *             if an I/O error occurs
	 ...
	 *
	 * @param request
	 *            the <tt>Request</tt> which has been attempted in order to
	 *            install a <tt>Channel</tt>
	 * @param bound
	 *            <tt>true</tt> if the <tt>bound</tt> property of the
	 *            <tt>Channel</tt> is to be set to <tt>true</tt>; otherwise,
	 *            <tt>false</tt>
	 */
	 ...
	 *
	 * @param request
	 *            the <tt>Request</tt> which has been sent to allocate a
	 *            specific channel number for a <tt>Channel</tt>
	 * @param channelNumberIsConfirmed
	 *            <tt>true</tt> if the channel number has been successfully
	...
	private class Channel {
		 ...
		 * <tt>RelayedCandidateDatagramSocket</tt> with a permission to send to
		 * a specific peer <tt>TransportAddress</tt>.
		 *
		 * @param peerAddress
		 *            the <tt>TransportAddress</tt> of the peer to which the new
		 *            instance is to provide a permission of this
		 *            <tt>RelayedCandidateDatagramSocket</tt> to send data to
		 */
		 ...
		 *             if anything goes wrong while binding/installing this
		 *             channel
		 */
		public void bind() throws StunException {
			byte[] createPermissionTransactionID = TransactionID.createNewTransactionID().getBytes();
			Request createPermissionRequest = MessageFactory.createCreatePermissionRequest(peerAddress,
		 ...
		 *
		 * @param channelNumber
		 *            the channel number to be compared to the channel number of
		 *            this <tt>Channel</tt> for value equality
		 ...
		 *         itself and has not received a confirmation that it has
		 *         succeeded in doing so yet; otherwise, <tt>false</tt>
		 */
		...
		public boolean isBound() {
			if ((bindingTimeStamp == -1)
					|| (bindingTimeStamp + PERMISSION_LIFETIME - PERMISSION_LIFETIME_LEEWAY) < System
							.currentTimeMillis())
				return false;
		 ...
		 *
		 * @param peerAddress
		 *            the <tt>TransportAddress</tt> which is to be checked for
		 *            equality (as defined by this <tt>Channel</tt> and not
		 ...
		 *            the <tt>DatagramPacket</tt> to be sent
		 * @param peerAddress
		 *            the <tt>TransportAddress</tt> of the peer to which the
		 *            <tt>DatagramPacket</tt> is to be sent
		 * @throws StunException
		 *             if anything goes wrong while sending the specified
		 *             <tt>DatagramPacket</tt> to the specified peer
		 *             <tt>TransportAddress</tt>
		 */
		public void send(DatagramPacket p, TransportAddress peerAddress) throws StunException {
			...
			if (channelDataIsPreferred && (channelNumber != CHANNEL_NUMBER_NOT_SPECIFIED) && channelNumberIsConfirmed) {
				...
				// Application Data
				System.arraycopy(data, 0, channelData, CHANNELDATA_CHANNELNUMBER_LENGTH + CHANNELDATA_LENGTH_LENGTH,
						length);

				try {
					if (channelDataPacket == null) {
						channelDataPacket = new DatagramPacket(channelData, 0, channelDataLength,
					...
					throw new StunException(StunException.NETWORK_ERROR, "Failed to send TURN ChannelData message",
							ioex);
				}
			} else {
				...
				turnCandidateHarvest.harvester.getStunStack().sendIndication(sendIndication,
						turnCandidateHarvest.harvester.stunServer,
		 ...
		 *
		 * @param bound
		 *            <tt>true</tt> if this <tt>Channel</tt> is to be marked as
		 *            bound/installed; otherwise, <tt>false</tt>
		 * @param boundTransactionID
		 *            an array of <tt>byte</tt>s which represents the ID of the
		 *            transaction with which the confirmation about the
		 ...
		 * messages instead of Send indications.
		 *
		 * @param channelDataIsPreferred
		 *            <tt>true</tt> if this <tt>Channel</tt> is to be set to
		 *            prefer sending <tt>DatagramPacket</tt>s using TURN
		 *            ChannelData messages instead of Send indications
		 */
		 ...
		 *
		 * @param channelNumberIsConfirmed
		 *            <tt>true</tt> if allocation of the channel number has been
		 *            confirmed by a success <tt>Response</tt> to our
		 *            ChannelBind <tt>Request</tt>
		 * @param channelNumberIsConfirmedTransactionID
		 *            an array of <tt>byte</tt>s which represents the ID of the
		 *            transaction with which the confirmation about the
		 *            allocation of the channel number has arrived
		 */
		public void setChannelNumberIsConfirmed(boolean channelNumberIsConfirmed,
