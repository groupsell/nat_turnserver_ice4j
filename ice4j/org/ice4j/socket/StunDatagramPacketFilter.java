...
public class StunDatagramPacketFilter implements DatagramPacketFilter {
	 ...
	 * <tt>DatagramPacket</tt>s which represent STUN messages received from any
	 * destination.
	 */
	 ...
	 *
	 * @param stunServer
	 *            the <tt>TransportAddress</tt> of the STUN server
	 *            <tt>DatagramPacket</tt>s representing STUN messages from and
	 *            to which will be accepted by the new instance or <tt>null</tt>
	 *            if we would like to accept stun messages from any destination.
	 */
	 ...
	 *            is a STUN message which is part of the communicator with the
	 *            STUN server associated with this instance
	 * @return <tt>true</tt> if the specified <tt>DatagramPacket</tt> represents
	 *         a STUN message which is part of the communication with the STUN
	 *         server associated with this instance; otherwise, <tt>false</tt>
	 */
	 ...
	 *            <tt>DatagramPacket</tt> to be checked whether it is accepted
	 *            by this <tt>DatagramPacketFilter</tt>
	 ...
	 *            STUN server associated with this instance
	 *
	 * @return True if the <tt>DatagramPacket</tt> represents a STUN (or TURN)
	...
	public static boolean isStunPacket(DatagramPacket p) {
		...
		if (length >= 20) {
			...
			// compliant).
			if (data[offset + 4] == Message.MAGIC_COOKIE[0] && data[offset + 5] == Message.MAGIC_COOKIE[1]
					&& data[offset + 6] == Message.MAGIC_COOKIE[2] && data[offset + 7] == Message.MAGIC_COOKIE[3]) {
				isStunPacket = true;
			}
