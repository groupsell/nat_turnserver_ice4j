...
class NetAccessManager implements ErrorHandler {
	 ...
	 * <tt>TransportAddress</tt> and and a remote <tt>TransportAddress</tt> to a
	 * <tt>Connector</tt>. We allow a <tt>Connector</tt> to be added without a
	 * specified remote address, under the <tt>null</tt> key.
	 *
	 ...
	 *
	 * @param stunStack
	 *            the <tt>StunStack</tt> which is creating the new instance, is
	 *            going to be its owner and is the handler that incoming message
	 *            requests should be passed to
	 */
	 ...
	 */
	NetAccessManager(StunStack stunStack, PeerUdpMessageEventHandler peerUdpMessageEventHandler,
	...
	/**
	 * Gets the <tt>BlockingQueue</tt> of this <tt>NetAccessManager</tt> in
	...
	/**
	 * Creates and starts a new access point based on the specified socket. If
	 * the specified access point has already been installed the method has no
	 * effect.
	 *
	 * @param socket
	 *            the socket that the access point should use.
	 */
	...
	protected void addSocket(IceSocketWrapper socket, TransportAddress remoteAddress) {
		...
		TransportAddress localAddress = new TransportAddress(socket.getLocalAddress(), socket.getLocalPort(),
				transport);

		final Map<TransportAddress, Map<TransportAddress, Connector>> connectorsMap = (transport == Transport.UDP)
				? udpConnectors : tcpConnectors;
		...
		synchronized (connectorsMap) {
			...
			if (!connectorsForLocalAddress.containsKey(remoteAddress)) {
				...
				logger.info("Not creating a new Connector, because we already "
						+ "have one for the given address pair: " + localAddress + " -> " + remoteAddress);
			}
		}
	}
	 ...
	 *
	 * @param localAddress
	 *            the local address of the connector to remove.
	 * @param remoteAddress
	 *            the remote address of the connector to remote. Use
	 *            <tt>null</tt> to match the <tt>Connector</tt> with no
	 *            specified remote address.
	 */
	protected void removeSocket(TransportAddress localAddress, TransportAddress remoteAddress) {
		Connector connector = null;

		final Map<TransportAddress, Map<TransportAddress, Connector>> connectorsMap = (localAddress
				.getTransport() == Transport.UDP) ? udpConnectors : tcpConnectors;
	...
	public void stop() {
		...
		for (Object o : new Object[] { udpConnectors, tcpConnectors }) {
			Map<TransportAddress, Map<TransportAddress, Connector>> connectorsMap = (Map<TransportAddress, Map<TransportAddress, Connector>>) o;

			synchronized (connectorsMap) {
				for (Map<TransportAddress, Connector> connectorsForLocalAddress : connectorsMap.values()) {
	 ...
	 *            the source address.
	 * @param remoteAddress
	 *            the destination address. Returns the <tt>Connector</tt>
	 *            responsible for a particular source address and a particular
	 *            destination address, or <tt>null</tt> if there's none.
	 */
	private Connector getConnector(TransportAddress localAddress, TransportAddress remoteAddress) {
		...
		final Map<TransportAddress, Map<TransportAddress, Connector>> connectorsMap = udp ? udpConnectors
				: tcpConnectors;
		Connector connector = null;
	...
	// --------------------------------
	/**
	 * Adjusts the number of concurrently running MessageProcessors. If the
	 * number is smaller or bigger than the number of threads currently running,
	 * then message processors are created/deleted so that their count matches
	 * the new value.
	 *
	 * @param threadPoolSize
	 *            the number of MessageProcessors that should be running
	 *            concurrently
	 ...
	 *            the message to send
	 * @param srcAddr
	 *            the access point to use to send the message
	 * @param remoteAddr
	 ...
	 *            the bytes to send.
	 * @param srcAddr
	 *            the access point to use to send the bytes.
	 * @param remoteAddr
	 *            the destination of the message.
	 *
	 * @throws IllegalArgumentException
	 ...
	 *             network socket.
	 */
	void sendMessage(byte[] bytes, TransportAddress srcAddr, TransportAddress remoteAddr)
			throws IllegalArgumentException, IOException {
